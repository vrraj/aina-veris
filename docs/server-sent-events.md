...
# SSE (Server-Sent Events) Architecture for Aina-Veris

> **About this document**
>
> This page explains the **Server-Sent Events (SSE) architecture** for the *Aina-Veris* system, including real-time streaming implementation, event formats, and integration details.
>
...
```bash
curl -N http://localhost:8100/chat/stream/stages?query_id=12345
```
...
response = requests.get('http://localhost:8100/chat/stream/stages?query_id=12345', stream=True)
