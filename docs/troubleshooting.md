...
> **About this document**
>
> This page provides **troubleshooting guidance** for the *Aina-Veris* system, including common issues, error solutions, and debugging steps.
>
...
This guide covers common issues, errors, and solutions for the aina-veris system.
...
**Issue:** Port 8100 or 6333 already in use

**Solutions:**
1. Find process using the port:
   ```bash
   lsof -i :8100  # or :6333 for Qdrant
   ```
...
1. Verify `ALLOWED_ORIGINS` in `.env`:
   ```bash
   echo "ALLOWED_ORIGINS=http://localhost:8100,http://127.0.0.1:8100" >> .env
   ```
...
2. Re-index the affected domain after changing embedding configuration
3. Use retrieval evaluation to compare results across configured models
...
# Run maintained retrieval and pipeline tests
.venv/bin/python -m pytest tests/test_retrieval_config.py tests/test_retrieval_orchestration.py
.venv/bin/python -m pytest tests/test_chat_pipeline_stages.py
...
1. Check the [GitHub Issues](https://github.com/vrraj/aina-veris/issues) for similar problems
