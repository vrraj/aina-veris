...
async function populateDomainDropdown() {
    ...
    try {
        ...
        // Set initial value from local browser preference, then backend runtime fallback.
        const localDomain = getActiveDomain();
        const currentDomain = localDomain && domains.includes(localDomain)
            ? localDomain
            : backendDomain;
        if (currentDomain) {
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Search is now handled by index-page.js (panel-based)
    // Debug index is now handled by index-page.js (panel-based)
    // Delete index is now handled by index-page.js (panel-based)
    ...
    // List docs is now handled by index-page.js (panel-based)
    openChatBtn && openChatBtn.addEventListener('click', () => window.open('/ask.html', '_blank'));
