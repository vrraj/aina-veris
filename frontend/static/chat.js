...
(function () {
  ...
  // Return the shared diagnostics area below metrics, creating it on demand.
  function ensureQueryDiagnosticsContainer() {
    const bar = document.getElementById('metrics_bar');
    if (!bar) return null;
    let container = document.getElementById('query_diagnostics');
    if (!container) {
      container = document.createElement('div');
      container.id = 'query_diagnostics';
      container.style.cssText = 'display:flex;flex-direction:column;gap:6px;margin:6px 0 10px;';
  ...
  function renderRewritePill(display) {
    try {
      const container = ensureQueryDiagnosticsContainer();
      if (!container) return;
  ...
  // Render the compound query plan produced before retrieval.
  function renderCompoundQueryPill(display) {
    try {
      const container = ensureQueryDiagnosticsContainer();
      if (!container) return;
      let pill = document.getElementById('compound_query_pill');
      const d = display || {};
      const queries = Array.isArray(d.queries) ? d.queries.filter(Boolean) : [];
      if (!d.enabled || !d.is_compound || queries.length < 2) {
        if (pill && pill.parentNode) pill.parentNode.removeChild(pill);
        return;
      }
      if (!pill) {
        pill = document.createElement('div');
        pill.id = 'compound_query_pill';
        pill.style.cssText = 'font-size:12px;background:#ecfeff;color:#164e63;border:1px solid #a5f3fc;border-radius:12px;padding:8px 10px;';
        container.appendChild(pill);
      }
      pill.innerHTML = '';
      const title = document.createElement('div');
      title.style.fontWeight = '600';
      title.textContent = 'Compound Query';
      const parent = document.createElement('div');
      parent.style.marginTop = '3px';
      parent.textContent = `Parent: "${truncate(String(d.normalized_query || d.original_query || ''), 180)}"`;
      const list = document.createElement('ol');
      list.style.cssText = 'margin:5px 0 0 20px;padding:0;';
      queries.forEach((query) => {
        const item = document.createElement('li');
        item.textContent = String(query);
        list.appendChild(item);
      });
      const meta = document.createElement('div');
      const pooling = d.pooling || {};
      const metaParts = [];
      if (d.rerank_label) metaParts.push(d.rerank_label);
      if (d.fusion_method) metaParts.push(`retrieval fusion: ${d.fusion_method}`);
      if (d.post_rerank_fusion) metaParts.push(`post-rerank fusion: ${d.post_rerank_fusion}`);
      if (pooling.pool_size != null && pooling.pool_cap != null) {
        metaParts.push(`pool: ${pooling.pool_size}/${pooling.pool_cap}`);
      }
      meta.style.cssText = 'margin-top:5px;opacity:.8;';
      meta.textContent = metaParts.join(' • ');
      pill.appendChild(title);
      pill.appendChild(parent);
      pill.appendChild(list);
      if (metaParts.length) pill.appendChild(meta);
    } catch (e) {
      console.debug('Failed to render compound query pill', e);
    }
  }
  ...
  function appendMessage(role, text, queryId) {
    ...
    bubble.dataset.queryId = queryId;
  ...
  function collectParams() {
    ...
    const streamAnswerEl = qs('#stream_answer');
    const stream_answer = streamAnswerEl ? !!streamAnswerEl.checked : false;
    ...
    const base = {
      ...
      split_compound_queries: (qs('#split_compound_queries') ? !!qs('#split_compound_queries').checked : false),
      max_compound_queries: getNum('max_compound_queries') || 4,
      compound_min_anchors_per_subquery: getNum('compound_min_anchors_per_subquery') || 5,
      compound_rerank_pool_cap: getNum('compound_rerank_pool_cap') || 40,
  ...
  async function sendMessage() {
    ...
    appendMessage('user', text, queryId);
   ...
   //bubble.textContent = 'Thinking…';
    ...
    await setupStageStreaming(queryId, bubble);
    ...
    try {
      ...
      if (bubble.dataset.queryId !== queryId) return;
      const answerText = (
      ...
      try { renderCompoundQueryPill(data && data.query_plan_display); } catch (_) {}
  ...
  if (clearChatBtn) {
    clearChatBtn.addEventListener('click', () => {
      ...
      try {
        ...
        const cp = document.getElementById('compound_query_pill');
      ...
      try {
        ...
        if (response.ok) {
          ...
          const mapping = {
            ...
            compound_min_anchors_per_subquery: 'compound_min_anchors_per_subquery',
            compound_rerank_pool_cap: 'compound_rerank_pool_cap'
          };
          ...
          // Use backend active_domain only when the browser has no remembered choice.
          try {
            const domainEl = document.getElementById('active_domain');
            const backendDomain = String(cfg.active_domain || '').trim();
            const localDomain = String(localStorage.getItem('active_domain') || '').trim();
            if (domainEl && backendDomain && !localDomain) {
...
function setupStageStreaming(queryId, bubbleEl) {
  if (!queryId) return Promise.resolve(false);
  ...
  try {
    ...
    const connectionTimeout = window.setTimeout(() => settleConnection(false), 1500);
    es.onopen = () => {
    ...
    const resolveBubble = () => {
      if (bubbleEl && document.body.contains(bubbleEl) && bubbleEl.dataset.queryId === queryId) return bubbleEl;
      const nodes = document.querySelectorAll('#chat_history .msg.assistant .bubble');
      return Array.from(nodes).find((node) => node.dataset.queryId === queryId) || null;
    };
    ...
    es.onmessage = (e) => {
      ...
      try {
        ...
        if (payload.stage === 'Answer Delta' && typeof payload.delta === 'string') {
          streamedText += payload.delta;
          if (!renderScheduled) {
            renderScheduled = true;
            window.requestAnimationFrame(renderStreamedText);
          }
          return;
        }
