...
const els = {
  ...
  discoveredTools: document.getElementById('discoveredTools'),
};
...
function renderDiscoveredTools() {
  const catalog = Array.isArray(registryData?.catalog_tools) ? registryData.catalog_tools : [];
  if (catalog.length === 0) {
    els.discoveredTools.innerHTML = '<tr><td colspan="4" class="py-3 text-gray-500">No tools are currently available. Use Reload Cache to discover enabled MCP servers.</td></tr>';
    return;
  }
  els.discoveredTools.innerHTML = catalog
    .map((tool) => `
      <tr>
        <td class="py-2 pr-4 font-mono text-xs text-gray-900">${escapeHtml(tool.name)}</td>
        <td class="py-2 pr-4 text-gray-700">${escapeHtml(tool.source)}</td>
        <td class="py-2 pr-4 text-gray-700">${tool.has_registry_override ? 'Yes' : 'No'}</td>
        <td class="py-2 text-gray-600">${escapeHtml(tool.description)}</td>
      </tr>`)
    .join('');
}
...
async function reloadToolCache() {
  ...
  const cleared = Number(data?.cache_entries_cleared ?? 0);
  await loadRegistry();
  setStatus(`Tool registry and MCP catalog reloaded. Tools available: ${Number(data?.tool_count ?? 0)}. Cache entries cleared: ${cleared}.`, 'success');
}
