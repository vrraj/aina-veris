...
from timeseries_svg import BarChartRenderer, TimeSeriesChartRenderer
...
def normalize_chart_type(chart_type: str | None) -> str:
    """Return a supported renderer name, preserving line charts as the default."""
    return "bar" if str(chart_type or "").strip().lower() == "bar" else "line"
...
def generate_timeseries_sparklines(
    ...
    *,
    ...
    renderer = (
        BarChartRenderer(**renderer_kwargs)
        if normalize_chart_type(chart_type) == "bar"
        else TimeSeriesChartRenderer(**renderer_kwargs)
    )
