...
def generate_stream(*, model_key: str, input: str, tools=None, **kwargs):
    """Generate text as an iterator of adapter-normalized stream events."""
    adapter = _get_adapter_for_model(model_key)
    filtered_kwargs = {
        key: value
        for key, value in kwargs.items()
        if key not in ["model", "input", "model_key", "stream"]
    }
