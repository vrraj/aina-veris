...
@dataclass(frozen=True)
class QueryDecompositionPromptSpec:
    system_instruction: str
    full_payload_template: str
...
def resolve_query_decomposition_prompt(
    *,
    registry_path: str,
    domain: Optional[str],
) -> QueryDecompositionPromptSpec:
    """Resolve the query-decomposition prompt and optional domain rules."""
    reg = _get_cached_yaml(registry_path)
    try:
        base_cfg = (reg.get("global_defaults") or {}).get("query_decomposition") or {}
    except Exception:
        base_cfg = {}

    base_sys = str(base_cfg.get("system_instruction") or "").strip()
    if not base_sys:
        raise LLMError(
            provider="internal",
            kind="config",
            message="Prompt registry missing required global_defaults.query_decomposition.system_instruction",
        )

    full_payload_template = ""
    for item in base_cfg.get("user_messages") or []:
        if isinstance(item, dict) and str(item.get("name") or "") == "full_payload":
            full_payload_template = str(item.get("template") or "")
            break
    if not full_payload_template.strip():
        raise LLMError(
            provider="internal",
            kind="config",
            message="Prompt registry missing required global_defaults.query_decomposition.user_messages[name=full_payload].template",
        )
    ...
    if dom:
        try:
            dom_cfg = ((reg.get("domains") or {}).get(dom) or {}).get("query_decomposition") or {}
            dom_sys = str(dom_cfg.get("system_instruction") or "").strip()
        except Exception:
            dom_sys = ""

    return QueryDecompositionPromptSpec(
        system_instruction=base_sys + ("\n\n" + dom_sys if dom_sys else ""),
        full_payload_template=full_payload_template,
    )
