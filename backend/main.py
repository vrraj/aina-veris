...
from backend.api.domain_indexing import (
    build_domain_qdrant as _build_domain_qdrant,
    get_embedding_rate_per_mm_tokens as _get_embedding_rate_per_mm_tokens,
    index_chunks_with_retrieval as _index_chunks_with_retrieval,
    resolve_domain_config as _resolve_domain_config,
...
async def app_lifespan(app: FastAPI):
    """Run MCP transport lifecycle and warm the outbound MCP tool catalog."""
...
# A2A remains a separate module while sharing this FastAPI process and port.
from backend.a2a.router import register_veris_a2a_routes

_a2a_request_handler = register_veris_a2a_routes(app)

app.add_middleware(MCPASGIMiddleware, mcp_handler=create_mcp_http_app())
...
async def chat_endpoint(session_id: str, chat_request: ChatRequest, request: Request):
    ...
    try:
        ...
        return ChatResponse(
            ...
            query_plan_display=response.get("query_plan_display", {}),
        )
    except Exception as e:
...
async def run_retrieval_evals(eval_request: RetrievalEvalRequest):
    ...
    try:
        return run_retrieval_eval(eval_request)
...
class ModelConfig(BaseModel):
    ...
    compound_min_anchors_per_subquery: Optional[int] = None
    compound_rerank_pool_cap: Optional[int] = None
    inference_context_rows: Optional[int] = None
    stream_answer: Optional[bool] = None
...
async def get_tool_registry():
    ...
    try:
        ...
        discovered_tools = get_discovered_mcp_tools()
        discovered_by_name = {tool["name"]: tool for tool in discovered_tools}
        catalog_tools: Dict[str, Dict[str, Any]] = {
            tool["name"]: {**tool, "has_registry_override": False}
        ...
        for entry in tools:
            ...
            discovered = discovered_by_name.get(name)
            catalog_tools[name] = {
                "name": name,
                "description": str(entry.get("description") or (discovered or {}).get("description") or ""),
                "source": str((discovered or {}).get("source") or endpoint.get("type") or "registry overlay"),
                "source_type": str((discovered or {}).get("source_type") or endpoint.get("type") or "registry"),
                "has_registry_override": True,
            }
        return {
            "registry_path": str(path),
            "registry": registry,
            ...
            "catalog_tools": sorted(catalog_tools.values(), key=lambda tool: tool["name"]),
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def update_tool_registry(payload: ToolRegistryUpdateRequest, request: Request):
    ...
    try:
        ...
        refresh_result = await asyncio.to_thread(refresh_tool_catalog)
        ...
        return {
            ...
            "cache_entries_cleared": int(
                refresh_result["registry_cache_entries_cleared"]
                + refresh_result["mcp_cache_entries_cleared"]
                + refresh_result["tool_list_entries_cleared"]
            ),
            "tool_count": len(refresh_result["tools"]),
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def reload_tool_registry_cache(request: Request):
    ...
    return {
        ...
        "tool_count": len(refresh_result["tools"]),
        "message": "Tool registry and MCP catalog reloaded.",
    }
...
@app.get(
    "/docs/tool_registry.html",
    tags=["1. UI Pages"],
    summary="Tool Registry documentation (HTML)",
    response_class=HTMLResponse,
)
async def tool_registry_documentation_page():
    """Serve the published Tool Registry documentation page."""
    docs_dir = Path(__file__).resolve().parent.parent / "docs"
    page_path = docs_dir / "tool_registry.html"
    if not page_path.exists():
        raise HTTPException(status_code=404, detail="Tool Registry documentation has not been published yet")
...
async def get_model_config():
    ...
    return ModelConfig(
        ...
        compound_min_anchors_per_subquery=get_model_setting(
            "compound_min_anchors_per_subquery"
        ),
        compound_rerank_pool_cap=get_model_setting("compound_rerank_pool_cap"),
        inference_context_rows=get_model_setting("inference_context_rows"),
        stream_answer=get_model_setting("stream_answer"),
    )
